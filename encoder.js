...
sources.forEach( source => {
    if ( source.type.match( 'Model|json' ) ) {

        // Read file
        fs.readFile('static/' + source.path, (err, data) => {
            if (err) throw err;
            ...
            const filePath = source.path.replace( /\.(glb|gltf|obj|json)/, '.bin' )

            fs.writeFile('static/' + filePath, uint8Array, (err) => {
                if (err) throw err;
